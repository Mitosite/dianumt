...
./$SHSCRIPTS/preprocessing.pairedend.sh
...
./$SHSCRIPTS/alignment.pairedend.sh -$LIST1 -N 1 -F 0.preprocessed.readsfile1.fastq -H 0.preprocessed.readsfile2.fastq -R human -1
wait
...
pairedmasterendtimetime=${SECONDS}
pairedmastertimedifference=`expr ${pairedmasterendtimetime} - ${pairedmasterstarttime}`
echo "master.pairedend.sh: took $pairedmastertimedifference seconds to run" >> $TIMELOG
